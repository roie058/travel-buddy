...
const HotelCard = (props: Props) => {
  ...
  return (
   ...
   {props.listItem? <Card sx={{width:'100%',overflow:'visible',minHeight:'120px',background:"rgba(96, 131, 255, 0.16)",backdropFilter:'blur(12.5px)'}}     >
       ...
       <Box sx={{borderRadius:0 , fontFamily: "Heebo, sans-serif",display:'flex',padding:'10px 10px',alignItems:'center'}}>
        ...
        <CardContent sx={{margin: '3%', marginRight:'0',padding:'0', paddingBottom:'0 !important',display:'flex',flexDirection:'column',gap:'10px'}}>
           ...
           {props.listItem.address&& <Typography width={"200px"} whiteSpace={"nowrap"} overflow={"hidden"} display={"inline-block"} textOverflow={"ellipsis"} ><Pin  height={10} width={10} />{props.listItem.address}</Typography>}
